...
const EditBookingModal = ({ booking, onClose, onSaved }) => {
  ...
  const [selectedTimeSlotId, setSelectedTimeSlotId] = useState(booking.timeSlotId?._id || '');
  const [assignSeat, setAssignSeat] = useState(Boolean(booking.seatId));
  const [halls, setHalls] = useState([]);
  const [activeHallId, setActiveHallId] = useState(booking.seatId?.hallId || null);
  const [seats, setSeats] = useState([]);
  const [selectedSeatId, setSelectedSeatId] = useState(booking.seatId?._id || null);
  const [loadingSeats, setLoadingSeats] = useState(false);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');

  useEffect(() => {
    timeSlotService.listTimeSlots().then(({ data }) => setTimeSlots(data.timeSlots));
  ...
  }, [assignSeat, activeHallId, selectedTimeSlotId, booking.startDate, booking.durationMonths]);
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl max-w-lg w-full p-5 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-gray-800">Edit {booking.studentId?.userId?.name}'s Booking</h3>
        ...
        {assignSeat && (
          <>
            ...
            {loadingSeats ? (
              ...
              <div className="grid grid-cols-8 gap-1.5 mb-4 max-h-52 overflow-y-auto">
                {seats.map((s) => (
                  <button
                    key={s._id}
                    type="button"
