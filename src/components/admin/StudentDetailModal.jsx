...
const StudentDetailModal = ({ student, onClose, onSaved }) => {
  ...
  useEffect(() => {
    Promise.all([
      bookingService.listBookings({ studentId: student._id, status: 'active' }),
      ...
      if (activeBooking) {
        ...
        setActiveHallId(activeBooking.seatId?.hallId || hallRes.data.halls[0]?._id);
  ...
  useEffect(() => {
    if (!assignSeat || !activeHallId || !selectedTimeSlotId || !booking) return;
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl max-w-2xl w-full p-5 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-gray-800">{student.userId?.name}</h3>
        ...
        <div className="flex items-center gap-4 mb-5">
          {form.photoUrl ? (
            <img src={form.photoUrl} alt="" className="h-16 w-16 rounded-full object-cover border border-gray-200" />
          ) : (
        ...
        {/* Profile fields */}
        <h4 className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Profile</h4>
        <div className="grid grid-cols-2 gap-3 mb-5">
          <div>
            <label className="text-xs text-gray-500 block mb-1">Date of Birth</label>
            <input type="date" value={form.dob} onChange={(e) => updateField('dob', e.target.value)} className="input-field w-full" />
          </div>
          <div>
          ...
          <div>
            ...
            <input value={form.bloodGroup} onChange={(e) => updateField('bloodGroup', e.target.value)} className="input-field w-full" />
          </div>
          <div>
            <label className="text-xs text-gray-500 block mb-1">Aadhaar Number</label>
            <input value={form.aadhaarNumber} onChange={(e) => updateField('aadhaarNumber', e.target.value)} className="input-field w-full" />
          </div>
          <div className="col-span-2">
            <label className="text-xs text-gray-500 block mb-1">Address</label>
            <textarea value={form.address} onChange={(e) => updateField('address', e.target.value)} rows={2} className="input-field w-full" />
          </div>
          <div>
            <label className="text-xs text-gray-500 block mb-1">Qualification</label>
            <input value={form.qualification} onChange={(e) => updateField('qualification', e.target.value)} className="input-field w-full" />
          </div>
          <div>
            <label className="text-xs text-gray-500 block mb-1">Preparing For</label>
            <input value={form.preparingFor} onChange={(e) => updateField('preparingFor', e.target.value)} className="input-field w-full" />
          </div>
          <div>
            <label className="text-xs text-gray-500 block mb-1">Father's Name</label>
            <input value={form.fatherName} onChange={(e) => updateField('fatherName', e.target.value)} className="input-field w-full" />
          </div>
          <div>
            <label className="text-xs text-gray-500 block mb-1">Mother's Name</label>
            <input value={form.motherName} onChange={(e) => updateField('motherName', e.target.value)} className="input-field w-full" />
          </div>
          <div className="col-span-2">
            <label className="text-xs text-gray-500 block mb-1">Parent Phone</label>
            <input value={form.parentPhone} onChange={(e) => updateField('parentPhone', e.target.value)} className="input-field w-full" />
          </div>
        </div>
        ...
        {loadingBooking ? (
          ...
          <p className="text-sm text-gray-400 italic">No active booking for this student.</p>
        ) : (
          <>
            <h4 className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">Seat & Shift</h4>
            <select
            ...
            <label className="flex items-center gap-2 mb-3 cursor-pointer">
              <input type="checkbox" checked={assignSeat} onChange={(e) => setAssignSeat(e.target.checked)} className="h-4 w-4 accent-brand rounded" />
              <span className="text-sm text-gray-700">Assign a fixed seat</span>
            </label>
            ...
            {assignSeat && (
              <>
                <div className="flex gap-2 mb-3 flex-wrap">
                  {halls.map((h) => (
                    <button
                      key={h._id}
                      type="button"
                      onClick={() => { setActiveHallId(h._id); setSelectedSeatId(null); }}
                ...
                {loadingSeats ? (
                  <p className="text-sm text-gray-400 mb-4">Loading seats...</p>
                ) : (
