...
const emptyForm = {
  ...
  dob: '', gender: 'male', bloodGroup: '', aadhaarNumber: '', address: '',
  qualification: '', preparingFor: '',
  durationMonths: 1, startDate: new Date().toISOString().slice(0, 10),
...
const AddStudent = () => {
  ...
  // Re-fetch the seat grid whenever hall, shift, duration, or start date
  // changes — availability is computed per-shift now, so any of these
  // changing can flip which seats are actually free.
  useEffect(() => {
  ...
  }, [activeHallId, selectedTimeSlotId, form.startDate, form.durationMonths]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!selectedTimeSlotId) return setError('Please select a time slot');
      ...
      <div className="max-w-lg mx-auto px-4 py-10 text-center">
        <div className="p-6 rounded-2xl bg-green-50 border border-green-200">
          ...
          <p className="text-sm text-green-600 mb-4">
            {success.user.name} · Seat {success.booking?.seatId?.seatNumber || selectedSeat?.seatNumber}
          </p>
          {success.receiptUrl && (
        ...
        <div className="flex gap-2 justify-center mt-5">
          <button onClick={() => { setSuccess(null); setForm(emptyForm); setSelectedSeat(null); }} className="btn-secondary text-sm px-4">
            Add Another
          </button>
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6">
      ...
      <p className="text-sm text-gray-500 mb-6">
        Creates their login, profile, seat booking, and sends a fee receipt on WhatsApp — all in one step.
      </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="card">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <input value={form.password} onChange={(e) => updateField('password', e.target.value)} placeholder="Set a login password" type="text" className="input-field" />
        ...
        {/* Shift, duration, date — now first, since seat availability depends on these */}
        <div className="card">
        ...
        {/* Seat picker — now shift-aware, refetches whenever shift/duration/date change above */}
        <div className="card">
          ...
          {loadingHalls ? (
            ...
            <>
                  ...
                  {seats.map((s) => (
                    <button
                      ...
                      className={`aspect-square rounded-lg border text-xs font-medium transition-colors ${
                        selectedSeat?._id === s._id ? 'bg-brand border-brand text-white' : STATUS_STYLE[s.status]
                      }`}
                    >
                      {s.seatNumber}
                    </button>
              ...
              {selectedSeat && (
                <p className="text-sm text-gray-500">Selected: Seat {selectedSeat.seatNumber}</p>
              )}
            </>
          )}
        </div>

        {/* Payment only now — shift/duration moved up above */}
        <div className="card">
        ...
        <button type="submit" disabled={submitting} className="btn-primary w-full py-2.5 disabled:opacity-50">
          {submitting ? 'Adding student...' : 'Add Student & Generate Receipt'}
        </button>
      </form>
    </div>
  );
